...
class Colourpop(QObject):
	...
	store = 'https://colourpop.com/'
	url_products = 'https://colourpop.com/products.json'
	url_product = 'https://colourpop.com/products/'
	url_add_js = 'https://colourpop.com/cart/add.js'
	url_cart = 'https://colourpop.com/cart/'
	url_shipping_rates = 'https://colourpop.com/cart/shipping_rates.json'
	url_payment_gateway_endpoint = 'https://deposit.us.shopifycs.com/sessions'
	url_start_checkout = 'https://colourpop.com/checkout'
	url_wallets = 'https://colourpop.com/13380845/digital_wallets/dialog'
	url_step = None
	payment_gateway = 94425479
	headers = {
		'authority': 'colourpop.com',
		'method': 'GET',
	...
	capture_folder = 'store_colourpop_test'
	...
	def submit_info(self, proxy=None):
		...
		phone = '({}) {}-{}'.format(self.profile.phone[0:3], self.profile.phone[3:6], self.profile.phone[6:])
	...
	def submit_shipping(self, proxy=None):
		...
		self.final_price = gecko_utils.get_final_price(r.text)
		title = str(soup.find('title'))
		print('    [TITLE]: {}'.format(title))
		self.auth_token = soup.find('input', {'name': 'authenticity_token'})['value']
		print('    [AUTH TOKEN]: {}'.format(self.auth_token))
	...
	def submit_payment(self, proxy=None):
		...
		if self.profile.same_as_shipping == QtCore.Qt.Checked:
			...
			payload = {
				...
				'checkout[total_price]': self.final_price,
				'complete': 1,
				'checkout[client_details][browser_width]': self.resolution['width'],
				'checkout[client_details][browser_height]': self.resolution['height'],
				'checkout[client_details][javascript_enabled]': 1,
